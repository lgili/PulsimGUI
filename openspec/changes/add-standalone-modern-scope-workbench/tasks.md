...
- [ ] 2.3 Support multiple signals per scope with drag/drop mapping and overlay/separate pane behavior
- [x] 2.4 Persist and restore per-scope independent state

## 3. Sidebar (Collapsible)

- [ ] 3.1 Implement left sidebar sections (scopes, signal catalog, trace manager, quick actions)
...
- [ ] 3.4 Ensure full keyboard navigation and accessible tooltips in collapsed mode
...
- [ ] 4.4 Implement interval selection targets (visible window, A, B, A→B)
- [x] 4.5 Ensure measurement values update in real time during cursor drag

## 5. PLECS-Inspired Analysis Features

- [ ] 5.1 Implement saved views list for named zoom/time ranges
- [ ] 5.2 Implement trace manager with visibility toggle, active trace selection, and color editing
- [ ] 5.3 Implement zoom overview mini-panel synced to the active plot window
- [ ] 5.4 Ensure UI behavior follows PLECS analysis flow while using modernized visual styling

## 6. Visual System and Theming

- [ ] 6.1 Define scope-specific design tokens (spacing, radius, color, typography)
- [ ] 6.2 Implement modern high-contrast style for charts, grid, cursor labels, and data table
- [ ] 6.3 Add subtle transitions for panel open/collapse and cursor panel reveal (< 180 ms)
- [ ] 6.4 Validate readability in light/dark themes and HiDPI scaling
...
- [ ] 7.5 Run validation locally: `PYTHONPATH=src QT_QPA_PLATFORM=offscreen pytest tests/`
