...
def test_3_phase_vsi_pins_and_defaults() -> None:
    ...
    # wire-binds the inverter via this pin) + 1 thermal-port pin (pulsim
    # 1.7 — wires the VSI's 6 internal switches into a SharedHeatsink as
    # a single composite device; the converter expands to 6 sub-devices).
    assert len(pins) == 7
    assert {p.name for p in pins} == {"VDC+", "VDC-", "A", "B", "C", "PWM", "TH"}
