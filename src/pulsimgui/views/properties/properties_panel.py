    ...
    def _create_param_widgets(self) -> None:
        ...
        for name, value in params.items():
            ...
            if widget:
                ...
                tooltip = self._get_tooltip_for_param(name)
                if tooltip:
...
"""
    ...
    def _get_unit_for_param(self, name: str) -> str:
        ...
        units = {
            ...
            "thermal_temp_init": "°C",
            "thermal_temp_ref": "°C",
            # pulsim 1.7 — per-device T_j safety limit + tempcos.
            "thermal_t_max_C": "°C",
            "thermal_t_max_hysteresis_C": "°C",
            "loss_a_cond_per_C": "1/°C",
            "loss_a_sw_per_C": "1/°C",
            # SharedHeatsink component (HEATSINK type).
            "R_th_sink_to_amb_K_per_W": "K/W",
            "C_th_sink_J_per_K": "J/K",
            "T_amb_C": "°C",
        }
        return units.get(name, "")

    def _get_tooltip_for_param(self, name: str) -> str:
        """Lookup table for descriptive tooltips on the new pulsim 1.7
        thermal fields.  Returns "" for params without a tooltip — the
        type-driven widget dispatch still shows the value, just without
        explanatory text on hover."""
        tooltips = {
            "thermal_t_max_C": (
                "Maximum junction temperature (°C). When the post-sim "
                "T_j(t) trace exceeds this value, the device is flagged "
                "in result.statistics['thermal_limit_trips']. Set to 0 "
                "to disable the check for this device."
            ),
            "thermal_t_max_hysteresis_C": (
                "Hysteresis band around T_max (°C). Prevents chatter "
                "when T_j hovers right at the limit. 0 = no hysteresis."
            ),
            "loss_a_cond_per_C": (
                "Temperature coefficient of conduction loss [1/°C]. "
                "Datasheet typicals: MOSFET ≈ +0.006 (Rds_on rises "
                "with T); diode ≈ −0.002 (V_f falls with T); IGBT ≈ "
                "+0.003. Non-zero values switch the coupled solver "
                "from shared_heatsink to electrothermal_steady_state "
                "(detects thermal runaway)."
            ),
            "loss_a_sw_per_C": (
                "Temperature coefficient of switching loss [1/°C]. "
                "Typical ≈ +0.001 — switching energies grow modestly "
                "with junction temperature. Anchored on the "
                "turn_on + turn_off reference power separately from "
                "loss_a_cond_per_C."
            ),
            "thermal_network": (
                "Topology of the per-device junction-to-case ladder. "
                "'single_rc' uses just thermal_rth + thermal_cth (one "
                "stage). 'foster' builds FosterStage(R, τ=R·C) from "
                "the CSV pairs (τ-based fit from datasheet Z_th(t)). "
                "'cauer' builds CauerStage(R, C) directly (per-layer "
                "physical values from FEA / Cauer measurement). At "
                "steady-state Foster and Cauer with the same R + C "
                "give identical T_j; transients differ."
            ),
            "thermal_rth_stages": (
                "Multi-stage Foster/Cauer resistances [K/W], comma- "
                "or semicolon-separated. Generated by the Foster Fit "
                "dialog from a datasheet Z_th(t) curve. Pairs with "
                "thermal_cth_stages — same length, same order."
            ),
            "thermal_cth_stages": (
                "Multi-stage Foster/Cauer capacitances [J/K], comma- "
                "or semicolon-separated. For Foster these encode τ = "
                "R · C; for Cauer they're physical per-layer C_th."
            ),
            # HEATSINK component params.
            "R_th_sink_to_amb_K_per_W": (
                "Heatsink-to-ambient thermal resistance [K/W]. "
                "Compute from sink area + airflow via Simulation → "
                "Thermal Sizing… (Convection tab). Typical: "
                "5–20 K/W natural convection, 1–5 K/W with 1–5 m/s "
                "airflow."
            ),
            "C_th_sink_J_per_K": (
                "Heatsink thermal capacitance [J/K]. Used by the "
                "transient observer (live T_sink(t)); ignored by "
                "the steady-state solver. 0 = treat sink as massless."
            ),
            "T_amb_C": "Ambient (cold-plate) temperature [°C].",
            "case_to_sink_R_th_csv": (
                "Per-slot case-to-sink R_th [K/W], comma- or "
                "semicolon-separated. Position k carries the TIM/"
                "insulator for device on DEV_k. Blank slots default "
                "to 0 (bare metal). Compute from TIM material via "
                "Simulation → Thermal Sizing… (TIM tab)."
            ),
            "n_devices": (
                "Number of DEV pins exposed by this heatsink (1–8). "
                "Pin layout regenerates when you change this; wire "
                "each DEV_k to a device's TH pin."
            ),
        }
