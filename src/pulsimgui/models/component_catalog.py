...
COMPONENT_LIBRARY = {
    "Circuit": [
        ...
        {"type": ComponentType.PV_PANEL, "name": "PV Panel", "shortcut": ""},
        {"type": ComponentType.BATTERY, "name": "Battery", "shortcut": ""},
        ...
        # NOTE: BJT_NPN/BJT_PNP, THYRISTOR, TRIAC, RELAY, FUSE and
        # CIRCUIT_BREAKER are modelled (component.py) and render, but the
        # circuit converter does not lower them to the backend yet — they stay
        # out of the palette until they actually simulate.
    ],
    "Analog": [
        {"type": ComponentType.OP_AMP, "name": "Op-Amp", "shortcut": ""},
    ...
    "Signal & Control": [
        ...
        {"type": ComponentType.PID_CONTROLLER, "name": "PID", "shortcut": ""},
        ...
        {"type": ComponentType.DIFFERENTIATOR, "name": "Differentiator", "shortcut": ""},
        {"type": ComponentType.RATE_LIMITER, "name": "Rate Limiter", "shortcut": ""},
        {"type": ComponentType.HYSTERESIS, "name": "Hysteresis", "shortcut": ""},
        {"type": ComponentType.LOOKUP_TABLE, "name": "Lookup Table", "shortcut": ""},
        {"type": ComponentType.TRANSFER_FUNCTION, "name": "Transfer Fn", "shortcut": ""},
        {"type": ComponentType.DELAY_BLOCK, "name": "Delay", "shortcut": ""},
        {"type": ComponentType.SAMPLE_HOLD, "name": "Sample/Hold", "shortcut": ""},
        {"type": ComponentType.STATE_MACHINE, "name": "State Machine", "shortcut": ""},
...
QUICK_ADD_COMPONENTS = [
    ...
    (ComponentType.PV_PANEL, "PV Panel",
     ["pv", "solar", "panel", "photovoltaic", "painel", "fotovoltaico",
      "mppt", "irradiance"]),
    (ComponentType.BATTERY, "Battery",
     ["battery", "bateria", "cell", "ocv", "soc", "storage", "48v"]),
    ...
    (ComponentType.PID_CONTROLLER, "PID Controller",
     ["pid", "controller", "derivative"]),
    (ComponentType.INTEGRATOR, "Integrator", ["integrator", "integral", "1/s"]),
    (ComponentType.DIFFERENTIATOR, "Differentiator",
     ["differentiator", "derivative", "d/dt"]),
    (ComponentType.RATE_LIMITER, "Rate Limiter",
     ["rate", "slew", "ramp limit"]),
    (ComponentType.HYSTERESIS, "Hysteresis",
     ["hysteresis", "bang bang", "relay control", "histerese"]),
    (ComponentType.LOOKUP_TABLE, "Lookup Table",
     ["lookup", "lut", "table", "interpolation", "curve"]),
    (ComponentType.TRANSFER_FUNCTION, "Transfer Function",
     ["transfer", "tf", "s-domain", "laplace", "filter"]),
    (ComponentType.DELAY_BLOCK, "Delay",
     ["delay", "dead time", "transport", "atraso"]),
    (ComponentType.SAMPLE_HOLD, "Sample & Hold",
     ["sample", "hold", "zoh", "s/h"]),
    (ComponentType.STATE_MACHINE, "State Machine",
     ["state machine", "fsm", "sequencer", "maquina de estados"]),
    (ComponentType.OP_AMP, "Op-Amp",
     ["opamp", "op-amp", "operational amplifier", "amplificador"]),
    (ComponentType.COMPARATOR, "Comparator",
     ["comparator", "comparador", "threshold"]),
    (ComponentType.POWER_PROBE, "Power Probe",
     ["power", "probe", "watt", "potencia", "p=vi"]),
