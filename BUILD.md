...
# Output: dist/PulsimGui.app, dist/PulsimGui-0.6.7-macos.dmg
```

The macOS build creates:
1. `PulsimGui.app` - Standard macOS application bundle
2. `PulsimGui-0.6.7-macos.dmg` - Distributable disk image
...
# Output: dist/PulsimGui-0.6.7-x86_64.AppImage, dist/pulsimgui
```

The Linux build creates:
1. `pulsimgui` - Standalone executable
2. `PulsimGui-0.6.7-x86_64.AppImage` - Portable AppImage (recommended for distribution)
...
- Git tags starting with `v` (e.g., `v0.6.7`)
